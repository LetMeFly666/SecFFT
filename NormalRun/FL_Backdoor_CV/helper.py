        ...
        if helper.params["aggregate_all_layer"] == 1:
            ...
            for _, parms in model.named_parameters():
                if parms.requires_grad:
                    ...
                    mask_grad_list.append(
                        mask_flat.reshape(parms.grad.size()).to(args.device)
                    )
            ...
            for _, parms in model.named_parameters():
                if parms.requires_grad:
                    ...
                    l2_norm_l = torch.norm(
                        parms.grad.view(-1).clone().detach().to(args.device)
                    ) / float(len(parms.grad.view(-1)))
                ...
                if parms.requires_grad:
                    ...
                    if ratio == 1.0:
                        _, indices = torch.topk(
                            -1 * gradients, int(gradients_length * 1.0)
                        )
                    else:

                        _, indices = torch.topk(
                            -1 * gradients, int(gradients_length * ratio)
                        )
