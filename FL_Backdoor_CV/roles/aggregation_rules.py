...
def secfft(model_updates):
    ...
    # 计算特征值和特征向量
    product_matrix = updates_dct @ updates_dct.T  # product_matrix为对称矩阵？
    eigenvalues, eigenvectors = torch.linalg.eigh(product_matrix)
    max_eigenvalue, max_eigenvector = eigenvalues[-1], eigenvectors[:, -1]

    # 计算干净成分
    max_eigenvalue = max_eigenvalue.real.item()  # 确保为实数
    max_eigenvector = max_eigenvector.view(-1, 1)
    clean_updates = updates_dct.T @ max_eigenvector / math.sqrt(max_eigenvalue)
    clean_updates = clean_updates.reshape(1, -1)  # clean ingredient

    # 计算KL散度
    clean_updates = clean_updates.expand(updates_dct.shape[0], -1)
    clean_updates_prob = F.softmax(clean_updates, dim=1)
    updates_dct_log_prob = F.log_softmax(updates_dct, dim=1)
    kl_div = F.kl_div(updates_dct_log_prob, clean_updates_prob, reduction="none")
    kl_div_per_sample = kl_div.sum(dim=1).cpu().numpy()
    clusterer = hdbscan.HDBSCAN(min_cluster_size=5, metric="euclidean")
    clusters = clusterer.fit_predict(kl_div_per_sample.reshape(-1, 1))
    cluster_counts = Counter(clusters)
    max_cluster_label = max(cluster_counts, key=cluster_counts.get)
    max_cluster_indices = np.where(clusters == max_cluster_label)[0]
    max_cluster_updates = updates_dct[max_cluster_indices]
    aggregated_updates = torch.mean(max_cluster_updates, dim=0)
...
def roseagg(model_updates, current_round=0):
    ...
    distance_matrix = smp.cosine_similarity(
        indicative_layer_updates.cpu().numpy()
    ) - np.eye(K)

    # === clustering using dbscan
    partition = DBSCAN(eps=args.threshold, min_samples=1, metric="cosine").fit(
        indicative_layer_updates.cpu().numpy()
    )
    clusters = dict()
    ...
    print(
        f"Clients with similar directional contribution: {sim_idxs}, "
    ...
    for name, layer_updates in model_updates.items():
        if "num_batches_tracked" in name:
            if args.is_poison:
                ...
                ) / len(layer_updates[args.number_of_adversaries])
            else:
            ...
            evals_large, evecs_large = largest_eigh(
                X.detach().cpu().numpy(), subset_by_index=(N - N, N - 1)
            )
            ...
            new_weights = torch.pow(
                torch.matmul(principal_direction.view(1, -1), origin_directions.T), 2
            )
            new_weights = new_weights / new_weights.sum()

            # === aggregation ===
            origin_directions += torch.normal(0, 0.003, origin_directions.size()).to(
                args.device
            )
...
def fltrust(model_updates, param_updates, clean_param_update):
    ...
    for name, params in model_updates.items():
        if (
            ...
            global_update[name] = (
                1 / nonzero_weights * params[nonzero_indices].sum(dim=0, keepdim=True)
            )
        else:
