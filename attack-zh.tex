...
\For {Client $i$}
    ...
    \EndFor
    \State Determine $\kappa$-distance $\widetilde{dis_i^\kappa}$ for $O_i$
    \State Find neighbors $Nei_i$ within $\widetilde{dis_i^\kappa}$
    \State Compute reachability distance $rea_{i,j} = \max\{\widetilde{dis_i^\kappa}, \|O_i - O_j\|\}$ for $O_j \in Nei_i$
