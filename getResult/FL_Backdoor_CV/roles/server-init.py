...
class Server:
    def __init__(self, helper, clients, adversary_list):
        ...
        self.benign_indices = list(
            set(list(range(args.participant_population))) - set(self.adversary_list)
        )
        ...
        if self.is_poison:
            ...
            if args.poison_rounds:
                ...
                retrain_rounds = np.arange(
                    self.current_round + 1 + args.windows,
                    self.current_round + args.windows + args.retrain_rounds + 1,
                )
                whether_poison = np.random.uniform(0, 1, args.retrain_rounds) >= (
                    1 - args.poison_prob
                )
    ...
    def select_participants(self):
        ...
        if args.random_compromise:
            self.participants = random.sample(
                range(args.participant_population), args.participant_sample_size
            )
        else:
            if self.current_round in self.poison_rounds:
                if args.attack_mode.lower() == "dba":
                    candidates = list()
                    adversarial_index = (
                        self.poison_rounds.index(self.current_round)
                        % args.dba_trigger_num
                    )
                    for client_id in self.adversary_list:
                        if (
                            self.clients[client_id].adversarial_index
                            == adversarial_index
                        ):  # DBA下每轮参与攻击的只有具有同样adversarial_index的攻击者
                            candidates.append(client_id)
                    self.participants = candidates + random.sample(
                        self.benign_indices,
                        args.participant_sample_size - len(candidates),
                    )
                    ...
                    self.participants = self.adversary_list + random.sample(
                        self.benign_indices,
                        args.participant_sample_size - len(self.adversary_list),
                    )
                ...
                self.participants = random.sample(
                    self.benign_indices, args.participant_sample_size
                )
                # === calculate the size of participating examples ===
                for client_id in self.participants:
                    self.total_size += self.clients[client_id].local_data_size
    ...
    def train_and_aggregate(self, global_lr, round, root_path):
        ...
        for client_id in self.participants:
            ...
            trained_local_model = self.clients[client_id].local_train(  # 评价时可注释
    ...
    def fedcie(model_updates, previous_model_update=None, last_model_params=None):
        ...
        distance_matrix = smp.cosine_similarity(
            last_layer_updates.cpu().numpy()
        ) - np.eye(K)

        """
        clustering distance matrix using dbscan
        """
        cluster = DBSCAN(eps=args.threshold, min_samples=1, metric="cosine").fit(
            last_layer_updates.cpu().numpy()
        )
        clusters = dict()
        ...
        print(
            f"Possible malicious indices: {mal_idxs}, "
        ...
        for name, layer_updates in model_updates.items():
            ...
            if "num_batches_tracked" in name:
                if args.is_poison:
                    ...
                    if layer_updates_0.sum() / len(
                        layer_updates_0
                    ) > layer_updates_1.sum() / len(layer_updates_1):
                        global_update[name] = torch.sum(layer_updates_1) / len(
                            layer_updates_1
                        )
                    else:
                        global_update[name] = torch.sum(layer_updates_0) / len(
                            layer_updates_0
                        )
                else:
                ...
                origin_directions = (
                    weights.view(-1, 1).to(args.device) * origin_directions
                )
                first_idxs = []
                if mal_idxs:
                    ...
                    for idxs in mal_idxs:
                        ...
                        for idx in idxs[1:]:
                            origin_directions[first_idx] = (
                                origin_directions[first_idx] + origin_directions[idx]
                            )
                        origin_directions[first_idx] /= torch.norm(
                            origin_directions[first_idx]
                        )
                            ...
                            if "running" not in name:
                                origin_directions[first_idx] = (
                                    ...
                                    * F.normalize(previous_model_update[name])
                                    + (1 - args.correction_coe)
                                    * origin_directions[first_idx]
                                )

                                origin_directions[first_idx] = origin_directions[
                                    first_idx
                                ] / torch.norm(origin_directions[first_idx])
                    ...
                    if "running" not in name:
                        for i in range(len(origin_directions)):
                            if i not in first_idxs:
                                ...
                                origin_directions[i] = origin_directions[
                                    i
                                ] / torch.norm(origin_directions[i])
                ...
                if args.cia_evaluation:
                    ...
                    evals_large, evecs_large = largest_eigh(
                        X.detach().cpu().numpy(), eigvals=(N - N, N - 1)
                    )
                    evals_large = torch.tensor(evals_large)[-1].to(args.device)
                    evecs_large = torch.tensor(evecs_large)[:, -1].to(args.device)
                    principal_direction = torch.matmul(
                        evecs_large.view(1, -1), origin_directions
                    ).T / torch.sqrt(evals_large)
                    ...
                    for i in range(N):
                        scale += (
                            normalize_norm
                            * cos(
                                principal_direction, origin_directions[i].view(-1, 1)
                            ).item()
                        )
    ...
    def roseagg_old(model_updates, important_feature_indices=None, current_round=0):
        ...
        for name, layer_updates in model_updates.items():
            if "num_batches_tracked" in name:
                global_update[name] = torch.sum(
                    layer_updates[possible_benign_idxs]
                ) / len(layer_updates[possible_benign_idxs])
            else:
                ...
                if args.cia_evaluation:
                    ...
                    new_weights = torch.pow(
                        torch.matmul(
                            principal_direction.view(1, -1), origin_directions.T
                        ),
                        2,
                    )
                    ...
                    origin_directions += torch.normal(
                        0, 0.003, origin_directions.size()
                    ).to(args.device)
                    origin_directions = F.normalize(origin_directions)
